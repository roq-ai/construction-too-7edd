...
import { useRoqClient, useSalesAssociateFindFirst } from 'lib/roq';
...
function SalesAssociateEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSalesAssociateFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SalesAssociateInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.sales_associate.update({
        data: values as RoqTypes.sales_associate,
        where: {
          id,
        },
      });
